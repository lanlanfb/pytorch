...
AT_ERROR("inverse: MAGMA library not found in "
  ...
  magma_int_t n = magma_int_cast(self.size(-2), "self.size(-2)");
  magma_int_t lda = std::max<magma_int_t>(1, n);
  magma_int_t lwork = std::max<magma_int_t>(1, n * magmaGetriOptimalBlocksize<scalar_t>(n));
...
TORCH_CHECK(false, "Calling torch.eig on a CUDA tensor requires compiling PyTorch with MAGMA. "
  ...
  if (n > 0) {
    ...
    magma_int_t lwork = std::max<magma_int_t>(1, real_impl<scalar_t, value_t>(wkopt));
...
AT_ERROR("torch.linalg.lstsq: MAGMA library not found in "
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(a.scalar_type(), "torch.linalg.lstsq_cuda", [&] {
    ...
    magma_int_t lwork = std::max<magma_int_t>(1, (m - n + nb) * (nrhs + nb) + nrhs * nb);
